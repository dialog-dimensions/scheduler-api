...
public class ShiftRepository : Repository<Shift>, IShiftRepository
{
    ...
    //     }
    //
    //     // Desk Navigation Property
    //     var deskMatches = (await _deskRepository.Query(parameters, prefixDiscriminator)).ToList();
    //     if (deskMatches.Any())
    //     {
    //         matches = matches.Where(shift => deskMatches.Select(d => d.Id).Contains(shift.DeskId));
    //     }
    //     
    //     // Schedule Navigation Property
    //     var scheduleMatches = (await _scheduleRepository.Query(parameters, prefixDiscriminator)).ToList();
    //     if (scheduleMatches.Any())
    //     {
    //         matches = matches.Where(shift =>
    //                 scheduleMatches.Select(s => s.StartDateTime).Contains(shift.ScheduleStartDateTime))
    //             .Where(shift => scheduleMatches.Select(s => s.DeskId).Contains(shift.DeskId));
    //     }
    //     
    //     // Employee Navigation Property
    //     var employeeMatches = (await _employeeRepository.Query(parameters, prefixDiscriminator)).ToList();
    //     if (employeeMatches.Any())
    //     {
    //         matches = matches
    //             .Where(shift => shift.EmployeeId.HasValue)
    //             .Where(shift => employeeMatches.Select(emp => emp.Id).Contains(shift.EmployeeId!.Value));
    //     }
    //     
    //
